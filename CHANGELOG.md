...
- **Convention de code établie : tout le code (entités, colonnes, données structurelles) est en anglais ; seuls les écrans visibles par les utilisateurs sont en français.** Concrètement :
  ...
  - La base stocke le `name` de l'enum (l'identifiant anglais), pas la `value` : nouveaux types Doctrine custom (`App\Doctrine\Types\EnumNameType` et ses 6 sous-classes, enregistrés dans `doctrine.yaml`) qui persistent/hydratent par `name` plutôt que par `value`. Trait `App\Enum\EnumNameLookup` (`fromName`/`tryFromName`) pour la recherche par name, utilisée notamment pour parser le vote pour/contre soumis en HTML brut par `/comite`.
